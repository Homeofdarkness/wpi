...
flowchart TD
    ...
    D --> E["SkipMoveRules"]
    D --> F["Чистые формулы"]
    D --> G["Бюджетный и демографический отчёты"]
...
- `stats/` содержит Pydantic-модели, ограничения входных данных, pretty-вывод
  и TOML-codec промышленности с импортом прежнего YAML v2. Формулы эффектов разбираются безопасным AST без
  `eval`.
...
- `PopulationGrowthBreakdown` сохраняет базовый прирост, ресурсную поправку,
  каждый демографический коэффициент, убыль и смерти от недоедания.
- `TurnCalendar` в `functions/time_models.py` является единственным источником
  длительности хода. `TURN_MONTHS` задаёт календарь, а шестимесячные нормы
  автоматически переводятся в поток текущего хода.
...
3. Обновляются сельское хозяйство и продовольствие.
4. Ресурсы добываются месячными подшагами по числу месяцев текущего хода.
5. Выполняются многoходовые производственные правила и потребление ресурсов.
6. Применяются ресурсные эффекты, после чего обновляется население.
7. Обновляются промышленность, налоги и торговля.
8. Доходы и расходы сводятся в `TurnLedger`.
9. Обновляются казна, стабильность, образование и военное оснащение.
10. Рассчитываются вероятности крупных событий за длительность хода.
11. При отрицательной казне интерфейс может предложить кредит.
...
Новый материал не требует изменения кода: страна регистрирует произвольный
alias, имя и одну из фиксированных групп в TOML. Цель эффекта может ссылаться
на любую числовую стату; движок безопасно разрешает её по Pydantic-модели и
выбирает этап применения. Неоднозначные имена уточняются именем раздела.
