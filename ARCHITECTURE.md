...
4. Шестью месячными подшагами обновляются ресурсы и склады.
...
10. Рассчитываются вероятности крупных событий за полугодие.
...
alias, имя и одну из фиксированных групп в YAML. Новая цель эффекта, напротив,
добавляется явно в `EffectTarget` и обработчик движка, потому что только код
может безопасно определить, когда и как изменяется конкретная стата.
