...
AGRICULTURE_COMMON_FIELDS = {
    ...
    "biome_richness": field(
        "biome_richness", "Богатство биомов", decimals=1, suffix="%"
    ),
    ...
    "sec2": list_item(
        ...
        decimals=1,
        suffix="%",
        aliases=("Орудия труда",),
    ),
    "workers_percent": field(
        "workers_percent", "Процент рабочих", decimals=1, suffix="%"
    ),
    "workers_redistribution": field(
        "workers_redistribution",
        "Рабочее перераспределение",
        decimals=1,
        suffix="%",
    ...
    "environmental_food": field(
        "environmental_food",
        "Еда из окружающей среды",
        decimals=0,
    ...
    "agriculture_deceases": field(
        "agriculture_deceases", "Хвори сельхоза", decimals=1, suffix="%"
    ),
    ...
    "overstock_percent": field(
        "overstock_percent", "Изъять из потребления", decimals=1, suffix="%"
    ),
    ...
    "food_supplies": field(
        "food_supplies", "Запасы пищи", decimals=2, default=0.0
    ),
}
...
AGRICULTURE_LAYOUT = PrettyLayoutSpec(
    ...
    lines=(
        ...
        PrettyLineSpec(
            fields=("biome_richness", "overprotective_effects"),
            line_width=145,
            min_gap=20,
        ),
...
ISF_AGRICULTURE_LAYOUT = PrettyLayoutSpec(
    fields={
        **AGRICULTURE_COMMON_FIELDS,
        "empire_land_unmastery": field(
            "empire_land_unmastery",
            "Неосвоенность Империи",
            decimals=1,
            aliases=("Неосвоенность земель Империи",),
        ),
    },
    lines=(
        PrettyLineSpec(title="СЕЛЬСКОЕ ХОЗЯЙСТВО"),
