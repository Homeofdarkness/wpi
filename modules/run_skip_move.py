...
from stats.isf_stats import IsfAgricultureStats, IsfEconomyStats, IsfIndustrialStats, IsfInnerPoliticsStats
...
class SkipMoverBase(ABC):
    ...
    InMoveFunctions: BaseInMoveFunctions = field(default_factory=BaseInMoveFunctions)
    ...
    def _update_stability(self, rules: SkipMoveRules, contentment_coefficient_2: float) -> Tuple[float, float]:
        ...
        logger.debug(f"Начальная экономическая стабильность - {buffed_stability}%")
        ...
        if (
            ...
            boost = self.InMoveFunctions.calculate_money_income_simple_boost(buffed_stability)
    ...
    def _update_education(self) -> None:
        ...
        if knowledge_diff < 0:
            ...
            self.InnerPolitics.education_level = round(
                max(expected_knowledge, self.InnerPolitics.education_level - reduction)
            )
            logger.debug(f"Образованность снижена на {reduction}")
        else:
            increase = abs(knowledge_diff) / max(self.InnerPolitics.education_level, 1)
    ...
    def _update_military_equipment(self) -> None:
        ...
        equipment_increase = self.Economy.war_wastes[1] * self.InMoveFunctions.calculate_military_equipment_coefficient(
...
class BasicSkipMove(SkipMoverBase):
    ...
    InMoveFunctions: BasicInMoveFunctions = field(default_factory=BasicInMoveFunctions)
    Rules: SkipMoveRules = field(default_factory=BasicSkipMoveRules)
    ...
    def run(self) -> SkipMoveReport:
        try:
            ...
            report = self._finalize_calculations(
                ...
                contentment_coefficient_2=float(results.contentment_coefficient_2),
            )
    ...
    def _perform_basic_calculations(self, logistic_wastes: float) -> CalculationResults:
        ctx = self._ctx()
        logistic_params: LogisticParams = self.Rules.calculate_logistic_params(ctx, logistic_wastes)
    ...
    def _calculate_income_and_expenses(self, results: CalculationResults, logistic_wastes: float) -> None:
        self._calculate_agriculture_stats(results.logistic_params.food_security_spotter)
    ...
    def _calculate_agriculture_stats(self, food_security_spotter: float) -> None:
        ...
        self.Agriculture.food_security *= 1 - (self.Agriculture.agriculture_natural_deceases / 100)
        self.Agriculture.food_security *= 1 - (self.Agriculture.agriculture_deceases / 100)

        # Mode-specific agriculture tweaks
        self.Rules.postprocess_agriculture(self._ctx())

    def _calculate_base_income(self, results: CalculationResults) -> None:
        income_multipliers = [
            self.InMoveFunctions.calculate_goods_coefficient(self.Industry.tvr1),
            (
    ...
    def _calculate_industry_stats(self) -> None:
        self.Industry.consumption_of_goods = self.InMoveFunctions.calculate_consumption_of_goods(
            self.Economy.population_count,
    ...
    def _calculate_tax_income(self, results: CalculationResults, logistic_wastes: float) -> None:
        ctx = self._ctx()
        self.Economy.tax_income = self.Rules.calculate_tax_income(ctx, results, logistic_wastes)
    ...
    def _calculate_trade_income(self) -> None:
        ...
        self.Economy.forex = self.InMoveFunctions.calculate_forex_course(
            ...
            self.Economy.trade_efficiency,
            round(self.Economy.trade_usage / self.Economy.trade_potential * 100),
    ...
    def _calculate_total_income(self, logistic_wastes: float) -> None:
        ...
        logger.debug(f"Коэффициент аллегоризации для торговли - {allegorization_trade_factor}")
        logger.debug(f"Коэффициент аллегоризации для остального - {allegorization_economy_factor}")
        ...
        science_income = getattr(self.InnerPolitics, "income_from_scientific", 0) or 0
        ...
        collaboration_factor = self.InMoveFunctions.calculate_money_income_collaboration_factor(
            self.Agriculture.agriculture_efficiency,
            self.Industry.civil_efficiency,
        )
        inflation_factor = self.InMoveFunctions.calculate_inflation_factor(self.Economy.inflation)
    ...
    def _finalize_calculations(
        ...
        *,
        ...
        budget_after_raw = budget_before + float(self.Economy.money_income) + float(logistic_discount)

        stability_after, boost = self._update_stability(self.Rules, contentment_coefficient_2)
        ...
        science_income = float(getattr(self.InnerPolitics, "income_from_scientific", 0) or 0)
...
class AtteriumSkipMove(BasicSkipMove):
    ...
    InMoveFunctions: AtteriumInMoveFunctions = field(default_factory=AtteriumInMoveFunctions)
...
class IsfSkipMove(BasicSkipMove):
    ...
    InMoveFunctions: IsfInMoveFunctions = field(default_factory=IsfInMoveFunctions)
